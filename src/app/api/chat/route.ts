...
export async function POST(req: Request) {
  try {
    ...
    // 2. Multimodal Image or Explicit Gemini Request -> Route to Google Gemini API
    if ((image || provider === 'gemini') && geminiKey) {
      try {
        return await callGeminiStream(messages, geminiKey, image, 'gemini-1.5-flash');
      } catch (geminiErr: any) {
    ...
    // 3. Fast Streaming with Groq API
    if (groqKey && provider !== 'gemini') {
      try {
        ...
        if (geminiKey) {
          return await callGeminiStream(messages, geminiKey);
        }
